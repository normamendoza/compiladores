...
void insertar(entrada e)
{
	...
	//printf("\nCompLex: %s  Lexema: %s",e.compLex,e.lexema);
	tabla[pos]=e;
...
char * element_list1 (char *synchset)
{
	...
	if (strstr(synchset,t.compLex)==NULL){
		if (strcmp (t.compLex, "COMA")==0){
			...
			if (strcmp (element_list1_trad, "R_CORCHETE") != 0)
   		    	strcat (trad, element_list1_trad);
			char * trad2 = trad;
			//printf ("trad2 %s", trad2);
			return trad2;
...
char * element_list (char *synchset)
{
	...
	if (strstr(synchset,t.compLex)==NULL){
		if (strcmp (t.compLex, "LITERAL_CADENA")==0 || strcmp (t.compLex, "L_CORCHETE")==0){
			...
			//printf ("\nResultado de element: %s",element_trad);
			//printf ("\nVa a element_list1()");
			char * element_list1_trad = element_list1("R_CORCHETE");
			//printf ("Estoy en element list esto es element_list1_trad: %s", element_list1_trad);
			//printf ("\nResultado de element_list1: %s",element_list1_trad);
			if (strcmp (element_list1_trad, "R_CORCHETE") != 0)
				strcat (trad, element_list1_trad);
			char * trad2 = trad;
			//printf ("\n***Traduccion element_list: %s",trad2);
			return trad2;
		}
		else 
			error2 ("element_list");
...
char * attribute_value (char *synchset)
{
	...
	if (strstr(synchset,t.compLex)==NULL){
		if (strcmp (t.compLex, "LITERAL_CADENA")==0){
			...
			//printf ("\n***Traduccion attribute_value: %s",cad);
			return cad;
		}
			...
			strcat (trad, "\"");
			char * aux = trad;
			//printf ("\n***Traduccion attribute_value: %s",aux);
			return aux;
		}
		else if (strcmp (t.compLex, "PR_TRUE")==0){
			match ("PR_TRUE");
			//printf ("\n***Traduccion attribute_value: True");
			return "true";	
		}
		else if (strcmp (t.compLex, "PR_FALSE")==0){
			match ("PR_FALSE");
			//printf ("\n***Traduccion attribute_value: False");
			return "false";	
		}
		else if (strcmp (t.compLex, "PR_NULL")==0){
			match ("PR_NULL");
			//printf ("\n***Traduccion attribute_value: ");
			return "";	
		}
		else 
...
char * attribute_name(char *synchset)
{
	...
	if (strstr(synchset,t.compLex)==NULL){
		if (strcmp (t.compLex, "LITERAL_CADENA")==0){
			char * cad = t.pe->lexema;
			match ("LITERAL_CADENA");
			//printf ("\n***Traduccion attribute_name: %s",cad);
			//return (SC (cad));
			return (sinComillas(cad));
		}
		else 
...
char * attribute(char *synchset)
{
	...
	if (strstr(synchset,t.compLex)==NULL){
		if (strcmp (t.compLex, "LITERAL_CADENA")==0){
			...
			//printf ("\nResultado de attribute_name: %s",attribute_name_trad);
			char trad [TAMLEX];
			strcpy (trad, attribute_name_trad);
			strcat (trad, "=");
			match ("DOS_PUNTOS");
			//printf ("\nVa a attribute_value()");
			char *attribute_value_trad = attribute_value("COMA R_LLAVE");
			//printf ("\nResultado de attribute_value: %s",attribute_value_trad);
			strcat (trad, attribute_value_trad);
			//trad[cant]='\0';
			char * trad2 = trad;
			//printf("\n+++Longitud trad2: %d",strlen (trad2));
			//printf ("\n***Traduccion attribute: %s",trad2);
			return trad2;
		}
		else 
			error2 ("attribute");
...
char * attributes_list (char *synchset)
{
	...
	if (strstr(synchset,t.compLex)==NULL){
		if (strcmp (t.compLex, "LITERAL_CADENA")==0){
			...
			//printf ("\nResultado de attribute: %s",attribute_trad);
			//printf("\n+++Longitud attribute: %d",strlen (attribute_trad));
			//printf ("\nVa a attributes_list1()");
			char * attributes_list1_trad = attributes_list1("R_LLAVE");
			//printf ("\nResultado de attributes_list1: %s",attributes_list1_trad);
			strcat (trad,attributes_list1_trad);
			char * attributes_list_trad = trad;
			//printf ("\n***Traduccion attributes_list: %s",attributes_list_trad);
...
char * attributes_list1 (char *synchset)
{
	char *primero = "COMA";
	//checkinput (primero,synchset);
	if (strstr(synchset,t.compLex)!=NULL){//Puse !=NULL 
		if (strcmp (t.compLex, "COMA")==0){
			match ("COMA");
			//printf ("\nVa a attribute()");
			char * attribute_trad = attribute("COMA R_LLAVE");
			...
			//printf ("\n***Traduccion attributes_list1: %s",attributes_list1_trad);
...
char * attributes1 (char *synchset)
{
	...
	if (strstr(synchset,t.compLex)==NULL){
		if (strcmp (t.compLex, "LITERAL_CADENA")==0){
			//printf ("\nVa a attributes_list()");
			char * attributes_list_trad = attributes_list("R_LLAVE");
			//printf ("\n***Traduccion attributes1: %s",attributes_list_trad);
			return attributes_list_trad;
		}
		checkinput(synchset, primero);
		return "";
...
{	
	...
	if (strstr(synchset,t.compLex)==NULL){
		if (strcmp (t.compLex, "L_LLAVE")==0){
			...
			match ("R_LLAVE");
			//printf ("\n***Traduccion attributes: %s",attributes1_trad);
...
char * tag_name (char *synchset)
{
	...
	if (strstr(synchset,t.compLex)==NULL){
		if (strcmp (t.compLex, "LITERAL_CADENA")==0){
			...
			//printf ("\n***Traduccion tag_name: %s",tag_name_trad);
...
char * element3 (char *synchset)
{
	...
	if (strstr(synchset,t.compLex)==NULL){
		...
		//printf ("\nResultado de element_list: %s",element_list_trad);
		char trad [TAMLEX];
		strcpy (trad, ",");
		strcat (trad, element_list_trad);
		char * element3_trad = trad;
		//printf ("\n***Traduccion element3: %s",element3_trad);
...
char * element (char *synchset)
{
	...
	if (strstr(synchset,t.compLex)==NULL){
		if (strcmp(t.compLex, "L_CORCHETE")==0)
		{
			match ("L_CORCHETE");
			//printf("\nVa a tag_name()");
			char * tag_name_trad = tag_name("COMA R_CORCHETE");;
			//printf ("\nResultado de tag_name: %s",tag_name_trad);
			//printf("\nVa a element1()");
			char trad [TAMLEX];
			strcpy (trad, "<");
			strcat (trad, sinComillas(tag_name_trad));
			char * element1_trad = element1 ("R_CORCHETE");
			//printf ("\nResultado de element1: %s",element1_trad);
			match ("R_CORCHETE");
			//printf("%s\n",trad);
			strcat (trad, element1_trad);
			strcat (trad, "</");
			strcat (trad, sinComillas(tag_name_trad));//ojo
			strcat (trad, ">");
			char *element_trad = trad;
			//printf ("\n***Traduccion element: %s",element_trad);
			return element_trad;
		}
		else if (strcmp(t.compLex, "LITERAL_CADENA")==0){
			char * cad = t.pe->lexema;
			match ("LITERAL_CADENA");
			//printf ("\n***Traduccion element: %s",cad);
			return sinComillas(cad);
		}
